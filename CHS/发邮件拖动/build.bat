...
:: 使用PyInstaller构建exe
pyinstaller -F -w -i icon.png -n %app_name% EmailSenderApp.py EmailSender.py ConfigManager.py FileListbox.py --distpath "%version_dir%" --workpath build_temp --specpath build_temp
...
if exist "%version_dir%\%app_name%.exe" (
    ...
    :: 复制配置文件
    if exist "邮箱对应表.xlsx" (
        xcopy "邮箱对应表.xlsx" "%version_dir%\" /Y /Q >nul
        echo   - 邮箱对应表.xlsx已复制
    )    
    if exist "config.json" (
        xcopy "config.json" "%version_dir%\" /Y /Q >nul
        echo   - config.json已复制
    )
